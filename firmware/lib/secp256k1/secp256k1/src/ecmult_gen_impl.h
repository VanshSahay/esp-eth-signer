...
static void secp256k1_ecmult_gen_context_build(secp256k1_ecmult_gen_context *ctx, void **prealloc) {
#ifndef USE_ECMULT_STATIC_PRECOMPUTATION
    /* ESP8266: these arrays are far too large for the 4 KB stack.
       Allocate on the heap instead.  Using plain malloc (rather than
       the library's checked_malloc) because this code is also compiled
       by gen_context.c which doesn't export the error callback. */
    const size_t num_entries = ECMULT_GEN_PREC_N * ECMULT_GEN_PREC_G;
    secp256k1_ge*  prec  = (secp256k1_ge*)  malloc(sizeof(secp256k1_ge)  * num_entries);
    secp256k1_gej* precj = (secp256k1_gej*) malloc(sizeof(secp256k1_gej) * num_entries);
    secp256k1_gej gj;
    ...
    if (!prec || !precj) {
        free(prec); free(precj);
        return;  // allocation failed — ctx->prec stays NULL
    }
#endif
